...
def plot_tsne(model, dataloader, device,title='',folder = 'tsne_plots/'):
    '''
    ...
    plt.title('t-SNE of Latent Space'+title)
    plt.savefig(f'tsne_plots/latent_tsne_{title}.png')
    plt.close()
    ...
    plt.colorbar(scatter)
    plt.title('t-SNE of Image Space'+title)
    plt.savefig(f'tsne_plots/image_tsne{title}.png')
    plt.close()